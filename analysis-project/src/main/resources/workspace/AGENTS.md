...
1. **query_data** — 数据查询专员
   - 用于：简单查询数据（“xxx指标是多少” ,尤其是问题包含“最值：最多、最少、最好等”、“排序”、“近期”、“明细/详情/清单”、“xxxx指标大于/小于xxxx数值”、“xxxx的数据"等特殊关键词。）
   - 支持查询维度：版本计划/季度、部门、应用/组/产品线、需求项、人员
   - 使用场景：用户只需要查数据，不需要分析时

2. **analyze_data** — 数据分析专家
   - 用于：数据分析需求（问题包含“xxx分析/情况/怎么样/如何/原因”等有分析意思的关键词。）
   - 该智能体内部会自动调用 tool_router 获取所需数据，无需预先查询
