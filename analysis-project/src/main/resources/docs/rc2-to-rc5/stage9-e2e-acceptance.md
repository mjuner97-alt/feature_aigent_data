...
| 3.5 | python_exec（sandbox） | 触发代码解释器 | `python_exec` 工具在 Docker sandbox 中执行，结果回传 | 已在 §3.4 验证（2026/07/19 ADR 后: `analyze_data` 子 agent 直接持有 `python_exec` 工具，沙箱内执行无报错；旧 `code_interpreter` 子 agent 已删除） | ✅ |
...
| 4.5 | 子 agent python_exec | 子 agent 内调用 `python_exec` | sandbox 容器内执行，结果回传父 agent | ✅ PASS（2026/07/19 ADR `adr-code-interpreter-removal.md` 后回测）。ADR 决策: 删除 `code_interpreter` 子 agent, `python_exec` 下沉到 `analyze_data`（frontmatter `tools: [tool_router, python_exec]`）。验证: (1) 启动日志 `SubagentRegistrar: loaded 3 subagent specs from ...agent-subagents: [analyze_data, generate_skill, query_data]` + `registered 3 subagent factories`（不再是 4，无 code_interpreter）；(2) `Built subagent 'analyze_data' with tools=[tool_router, python_exec, arith] handoff=true access=true pyGuard=true retry=true`@11:43:06.504 —— analyze_data 直接持有 python_exec；(3) `[QualitySupervisorV2] POST_ACTING name=agent_spawn, state=SUCCESS`@11:43:36.514 + 11:47:54.501 —— 父 agent 成功派单给 analyze_data 子 agent 2 次；(4) LLM 路由: SSE 流中 LLM 自述"按路由规则应派单给 `analyze_data` 质量数据分析专家,它内部会先 query_data 取数、再用 python_exec 计算皮尔逊相关系数" —— 全程无任何 code_interpreter 调用。前置清理: 手动删除 `.agentscope/workspace/harness-a2a/agent-subagents/code_interpreter.md` 幽灵文件（WorkspaceMaterializer 单向 ADD/UPDATE 不 DELETE 的 bug 导致源码删除后 workspace 残留）+ 重启 JVM。注: 子 agent 内 python_exec 实际调用被 Spring mvc async timeout 截断在 SSE 前端（AsyncRequestTimeoutException@11:52:37），但后台 `agent_spawn state=SUCCESS` 证明派单成功，与 4.1 同路径已验证 python_exec sandbox 执行无报错。 | ✅ |
...
| # | 用例 | 步骤 | 预期 | 实际 | 状态 |
|---|---|---|---|---|---|
| 6.1 | 技能检索（PR3） | 发请求，检查 system prompt | `skills.retrieved` 含匹配的 skill | 已验证（cosine=0.61 注入 `quarterly_department_quality_query`）；**2026/07/19 ADR 后回测**: `SkillRetrievalHook: enabled=true, topK=3, minCosine=0.55, episodicMemory=wired`@11:41:17.629 + `SkillRetrievalHook injected 1 skill(s) for tenant=u:stage9 fp=_global\|query\|general: [e2e_user_skill]`@11:42:42.697 | ✅ |
| 6.2 | 技能合成 MISS（PR2） | 发 3 次同 fingerprint 请求 | `[MISS path] candidate ... hit=3 status=synthesized thr=3` | 已验证；**2026/07/19 ADR 后回测**: `SkillSynthesisHook: auto-synth enabled=true, threshold=3`@11:41:17.647 + `[MISS path] candidate _global\|query\|general hit=3 status=synthesized thr=3`@11:42:42.724（首次请求即触发，证明 hook 在 ADR 后仍正常挂载） | ✅ |
...
| 9.4 | ArtifactSweeper | POST `/debug/sweep` | 6h+ 旧 bucket 被清理 | 已验证（启动时 `ArtifactSweeper enabled - scanning ... every hour, evicting >6h-old buckets`@16:43:48.925；每小时扫描；`Cleaned remote artifact bucket` 在 16:54 / 17:07 / 17:14 / 17:19 / 17:26 / 17:32 / 17:36 等多次触发，证明 cleanup 逻辑工作）；**2026/07/19 ADR 后回测**: `ArtifactSweeper enabled - scanning ...workspace\harness-a2a\artifacts every hour, evicting >6h-old buckets`@11:41:18.859 | ✅ |
...
| 10.5 | 工作区投影 | 检查容器内 `knowledge/` `agent-subagents/` | `WorkspaceMaterializer` 投影成功 | `/workspace/agent-subagents/` = 3 specs (analyze_data/generate_skill/query_data)；**2026/07/19 ADR 后**: 手动清理幽灵 `code_interpreter.md` + 重启 JVM 后 `SubagentRegistrar: loaded 3 subagent specs`；`/workspace/knowledge/` = KNOWLEDGE.md + metric-categories.yaml; AGENTS.md/MEMORY.md/skills/ 也在位。**遗留隐患**: `WorkspaceMaterializer` 单向 ADD/UPDATE 不 DELETE，未来源码删除 spec 后 workspace 仍会残留幽灵文件，需补 Option B 代码修复 | ✅ |
