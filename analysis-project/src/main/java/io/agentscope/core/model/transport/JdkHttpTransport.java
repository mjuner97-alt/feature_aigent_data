...
public class JdkHttpTransport implements HttpTransport {
    ...
    /**
     * SSE parsing uses blocking BufferedReader calls. Keep those calls away from
     * Reactor's shared boundedElastic pool, which is also used by database/docker
     * work in the application and can otherwise delay the first token indefinitely.
     */
    private static final reactor.core.scheduler.Scheduler SSE_SCHEDULER =
            Schedulers.newBoundedElastic(64, 10_000, "agentscope-sse");
    ...
    private JdkHttpTransport(HttpTransportConfig config, boolean gzipEnabled) {
    ...
    private JdkHttpTransport(
            HttpClient client, HttpTransportConfig config, boolean gzipEnabled) {
    ...
    public Flux<String> stream(HttpRequest request) {
        ...
        LlmFileTrace.write(traceId, "JdkHttpTransport", "请求构造", "url=" + request.getUrl() + " method=" + request.getMethod()
                + " timeout=" + config.getReadTimeout());
        var jdkRequest = buildJdkRequest(request);
        LlmFileTrace.write(traceId, "JdkHttpTransport", "开始发送", "url=" + request.getUrl());
                ...
                client.sendAsync(jdkRequest, BodyHandlers.ofInputStream())
                        .thenApply(
                                response -> {
                                    ...
                                    LlmFileTrace.write(traceId, "JdkHttpTransport", "收到响应头", "status=" + statusCode
                                            + " version=" + response.version() + " elapsedMs=" + elapsedMs(started)
                                            + " headers=" + response.headers().map().keySet());
        ...
        return Mono.fromCompletionStage(future)
                .flatMapMany(response -> processStreamResponse(response, request, traceId))
                .onErrorMap(
                        e -> !(e instanceof HttpTransportException),
                        e -> {
                            LlmFileTrace.write(traceId, "JdkHttpTransport", "异常", "elapsedMs=" + elapsedMs(started)
                                    + " type=" + e.getClass().getName() + " message=" + LlmFileTrace.shortText(e.getMessage()));
                ...
                .doOnComplete(() -> LlmFileTrace.write(traceId, "JdkHttpTransport", "完成", "elapsedMs=" + elapsedMs(started)))
                .doOnCancel(() -> LlmFileTrace.write(traceId, "JdkHttpTransport", "取消", "elapsedMs=" + elapsedMs(started)))
                .subscribeOn(SSE_SCHEDULER);
    }

    private Flux<String> processStreamResponse(
            java.net.http.HttpResponse<InputStream> response, HttpRequest request, String traceId) {
        ...
        return Flux.using(
                ...
                reader -> isNdjson ? readNdJsonLines(reader) : readSseLines(reader, traceId),
                this::closeQuietly);
    }

    private Flux<String> readSseLines(BufferedReader reader, String traceId) {
        return Flux.fromStream(reader.lines())
                ...
                .doOnNext(data -> LlmFileTrace.write(traceId, "JdkHttpTransport", "SSE数据",
    ...
    private java.net.http.HttpRequest buildJdkRequest(HttpRequest request) {
        ...
        switch (method) {
            case "GET":
                ...
                break;
            case "POST":
                builder.POST(bodyPublisher(body, builder));
                break;
            case "PUT":
                builder.PUT(bodyPublisher(body, builder));
                break;
            case "DELETE":
                builder.method("DELETE", bodyPublisher(body, builder));
                break;
            default:
                builder.method(method, bodyPublisher(body, builder));
        }
    ...
    private java.net.http.HttpRequest.BodyPublisher bodyPublisher(
            String body, java.net.http.HttpRequest.Builder builder) {
        if (body == null) {
            return java.net.http.HttpRequest.BodyPublishers.noBody();
        }
        if (!gzipEnabled || body.isEmpty()) {
            return java.net.http.HttpRequest.BodyPublishers.ofString(body, StandardCharsets.UTF_8);
        }

        byte[] raw = body.getBytes(StandardCharsets.UTF_8);
        byte[] compressed = gzip(raw);
        builder.setHeader("Content-Encoding", "gzip");
        log.debug("Compressed HTTP request body with gzip: rawBytes={}, compressedBytes={}",
                raw.length, compressed.length);
        return java.net.http.HttpRequest.BodyPublishers.ofByteArray(compressed);
    }
    ...
    private byte[] gzip(byte[] raw) {
        try {
            ...
            throw new HttpTransportException("Failed to gzip HTTP request body", e);
        }
    }
    ...
    public static class Builder {
        ...
        /**
         * Enable or disable gzip compression for non-empty HTTP request bodies.
         * Disabled by default because the target server must support
        ...
        public JdkHttpTransport build() {
            if (existingClient != null) {
                return new JdkHttpTransport(existingClient, config, gzipEnabled);
            }
            return new JdkHttpTransport(config, gzipEnabled);
        }
    }
