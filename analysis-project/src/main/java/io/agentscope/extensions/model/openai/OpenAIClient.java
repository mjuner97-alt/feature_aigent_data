...
public class OpenAIClient {
    ...
    public OpenAIResponse call(
        ...
        LlmFileTrace.write(traceId, "OpenAIClient", "同步请求开始", "url=" + url + " model=" + request.getModel());
        ...
        try {
            ...
            LlmFileTrace.write(traceId, "OpenAIClient", "请求体", LlmFileTrace.mediumText(requestBody));
    ...
    public Flux<OpenAIResponse> stream(
        ...
        String traceId = LlmFileTrace.id();
        long started = System.nanoTime();
        LlmFileTrace.write(traceId, "OpenAIClient", "流式请求开始", "url=" + url + " model=" + request.getModel());
                    ...
                    .<OpenAIResponse>handle(
                            (data, sink) -> {
                                ...
                                if (response != null) {
                                    ...
                                    LlmFileTrace.write(traceId, "OpenAIClient", "解析响应成功", "elapsedMs=" + elapsedMs(started));
                                    sink.next(response);
                                }
                                // If response is null (malformed chunk), skip it silently
                            })
                    .doOnComplete(() -> LlmFileTrace.write(traceId, "OpenAIClient", "完成", "elapsedMs=" + elapsedMs(started)))
                    .doOnCancel(() -> LlmFileTrace.write(traceId, "OpenAIClient", "取消", "elapsedMs=" + elapsedMs(started)))
                    .onErrorMap(
                            ex -> {
