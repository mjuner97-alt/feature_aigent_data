...
/**
 * 基于项目内置 Model 接口的 LlmDimensionService 实现。
 *
 * <p>适用于所有 OpenAI 兼容 API（包括火山引擎 Ark、DeepSeek、GLM 等），
 * 复用 AgentScope 已有的 Model 基础设施。
 *
 * <p>使用示例（火山引擎 Ark + GLM）：
 *
 * <pre>{@code
 * // 方式1：直接构造 GenerateOptions
 * GenerateOptions options = GenerateOptions.builder()
 *         .modelName("glm-5.1")
 *         .apiKey("ark-137faf76-3ebd-489d-8e45-0af23f723390-7995f")
 *         .baseUrl("https://ark.cn-beijing.volces.com/api/coding")
 *         .build();
 *
 * LlmDimensionService service = new OpenAILlmDimensionService(model, options);
 *
 * // 方式2：使用已有的 Model 实例
 * // 假设你已经在 Agent 中配置了 model，可直接复用
 * LlmDimensionService service = new OpenAILlmDimensionService(agentModel);
 * }</pre>
 */
