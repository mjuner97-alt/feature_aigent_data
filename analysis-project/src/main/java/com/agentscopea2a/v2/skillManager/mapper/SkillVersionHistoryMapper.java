...
/**
 * Skill 版本历史 Mapper - 维护 {@code skill_version_history} 表的版本快照写入与查询。
 * 包路径受 {@code MySQLConfig.@MapperScan(basePackages = "com.agentscopea2a.mapper.mysql")} 约束。
 */
@Mapper
public interface SkillVersionHistoryMapper {

    int insert(SkillVersionHistory history);

    List<SkillVersionHistory> selectBySkillId(@Param("skillId") Long skillId);
