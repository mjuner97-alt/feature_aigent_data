...
/**
 * Skill 审批记录 Mapper - 维护 {@code skill_approval} 表的逐次审批操作流水查询。
 * 包路径受 {@code MySQLConfig.@MapperScan(basePackages = "com.agentscopea2a.mapper.mysql")} 约束。
 */
@Mapper
public interface SkillApprovalMapper {

    int insert(SkillApproval approval);

    List<SkillApproval> selectByPublishId(@Param("publishId") Long publishId);

    List<SkillApproval> selectByDraftId(@Param("draftId") Long draftId);
}
