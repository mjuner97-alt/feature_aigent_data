 ...
 *
 * <p>写入根目录固定为 {@code /data/skill-files/{userId}/}，filePath 参数作为相对子路径，
 * 规范化后必须仍在该根目录内，防止路径穿越。
 *
...
public class WriteMarkdownTool {
    ...
    /** 写入根目录前缀，最终路径为 BASE_DIR_PREFIX + userId + "/" */
    private static final String BASE_DIR_PREFIX = "/data/skill-files/";

    /** 延迟获取的回调，SkillJobScheduler实现，用于标记md_file_written=true */
    private final Supplier<WriteCallback> writeCallbackSupplier;

    public WriteMarkdownTool(Supplier<WriteCallback> writeCallbackSupplier) {
     ...
     *
     * <p>最终写入路径为 {@code /data/skill-files/{userId}/{filePath}}，
    ...
    public boolean writeMarkdown(String filePath, String content, Long executionId, String userId) {
        try {
            ...
            // 解析根目录: /data/skill-files/{userId}/
            Path baseDir = Paths.get(BASE_DIR_PREFIX + userId).normalize().toAbsolutePath();
            // 解析目标文件: baseDir + filePath
            Path resolved = baseDir.resolve(filePath).normalize().toAbsolutePath();

            // 路径穿越防护：目标文件必须在 baseDir 内
            if (!resolved.startsWith(baseDir)) {
                log.warn("WriteMarkdownTool: path traversal detected: {} (base={})", filePath, baseDir);
                return false;
            }
            ...
            if (writeCallback != null) {
                writeCallback.onMarkdownWritten(resolved.toString(), executionId);
            }
