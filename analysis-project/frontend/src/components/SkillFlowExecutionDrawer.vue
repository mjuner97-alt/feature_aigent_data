...
import { getSkillFlowExecution, getSkillFlowExecutionMetrics, getSkillFlowExecutionNodes, getSkillFlowExecutionNotifications, getSkillFlowExecutionReportUrl, getSkillFlowNodeReportUrl, resendSkillFlowExecutionNotification, retrySkillFlowSummary, retrySkillFlowNode } from '../api/skillFlow';
...
async function load() {
  ...
  loading.value = true; error.value = ''; errorExpanded.value = {};
  try {
    ...
    const reportUrl = detail.reportPath ? await getSkillFlowExecutionReportUrl(props.executionId) : null;
    if (execution.value?.reportUrl?.startsWith('blob:')) URL.revokeObjectURL(execution.value.reportUrl);
    execution.value = { ...detail, metrics, nodes, notifications, reportUrl };
...
async function retrySummary() { if (!props.executionId) return; retrying.value = 'summary'; try { await retrySkillFlowSummary(props.executionId); await load(); emit('changed'); } catch (e) { error.value = e instanceof Error ? e.message : '重新生成汇总失败'; } finally { retrying.value = null; } }
async function retryNode(node: SkillFlowNodeExecution) { if (!props.executionId || !node.id) return; retrying.value = `node-${node.id}`; try { await retrySkillFlowNode(props.executionId, node.id); await load(); emit('changed'); } catch (e) { error.value = e instanceof Error ? e.message : '重跑任务失败'; } finally { retrying.value = null; } }
