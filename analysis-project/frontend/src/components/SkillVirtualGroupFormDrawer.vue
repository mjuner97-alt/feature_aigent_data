 ...
 * 虚拟组创建表单 Drawer
 *
 * 建组 = 组名 + 可选首个成员;空组合法,建组后可在"组成员"tab 继续补成员。
 */
...
async function search() {
  ...
  try {
    results.value = await searchSkillUsers(kw);
  } catch {
    results.value = [];
  } finally {
...
function selectMember(u: SkillUserSearchItem) {
  firstMember.value = u;
  keyword.value = u.name ? `${u.name} (${u.userId})` : u.userId;
  results.value = [];
}
...
async function submit() {
  ...
  saving.value = true;
  try {
    await createVirtualGroup(groupName.value.trim(), firstMember.value?.userId);
    close();
    emit('saved');
  } catch (e) {
    formError.value = e instanceof Error ? e.message : '建组失败';
  } finally {
    saving.value = false;
  }
}
            ...
            <form class="form" @submit.prevent="submit">
              ...
              <label class="field">
                ...
                <div v-if="results.length > 0" class="results">
                  <button v-for="u in results" :key="u.userId" type="button" class="user-opt" @click="selectMember(u)">
...
.form input { padding: 8px 10px; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 14px; font-family: inherit; background: #fff; }
...
.selected { font-size: 12px; color: #2563eb; display: flex; align-items: center; gap: 8px; }
.clear-btn { border: none; background: transparent; color: #94a3b8; font-size: 12px; cursor: pointer; padding: 0; text-decoration: underline; }
